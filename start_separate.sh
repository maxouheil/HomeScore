#!/bin/bash
# Script pour lancer le backend et le frontend dans des terminaux séparés
...
echo -e "${BLUE}🚀 Démarrage de HomeScore dans des terminaux séparés${NC}"
echo ""
...
EOF
        ...
        echo ""
        echo -e "${GREEN}✅ Backend et frontend lancés dans des terminaux séparés${NC}"
        echo ""
        ...
        echo -e "${RED}❌ osascript non disponible. Utilisez start.sh à la place.${NC}"
        exit 1
    fi
else
    ...
    if command -v gnome-terminal &> /dev/null; then
        ...
        echo -e "${RED}❌ Terminal non supporté. Utilisez start.sh à la place.${NC}"
        exit 1
    fi
fi
