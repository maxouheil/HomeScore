#!/usr/bin/env python3
"""
Scraper LeBonCoin utilisant l'API reverse engineered
...
class LeBonCoinScraper:
    """Scraper LeBonCoin avec support API et fallback HTML"""
    ...
    def __init__(self, use_api: bool = True, fallback_to_html: bool = True):
        ...
        self.api_client: Optional[LeBonCoinAPIClient] = None
    ...
    async def setup(self):
        ...
        if self.use_api:
            try:
                self.api_client = LeBonCoinAPIClient()
                print("✅ Client API LeBonCoin initialisé")
        ...
        if self.use_api and self.api_client:
            try:
                ...
                while len(self.properties) < max_results:
                    ...
                    if not result or not result.get('ads'):
                        break
                    ...
                    for ad_data in result['ads']:
                        prop = base_client.normalize_property_data(ad_data, 'leboncoin')
                        self.properties.append(prop)
                    
                    if len(result.get('ads', [])) < limit:
                        break
    ...
    async def _search_via_html(
        ...
        print("⚠️  Scraping HTML non implémenté pour LeBonCoin")
        ...
        if self.use_api and self.api_client:
            try:
                ...
                if result:
                    from api_client_base import BaseAPIClient
                    base_client = BaseAPIClient()
                    return base_client.normalize_property_data(result, 'leboncoin')
            except Exception as e:
