# Guide : Reverse Engineer l'API Privée SeLoger

Ce guide explique comment utiliser les scripts créés pour reverse engineer l'API privée de SeLoger.
...
2. ✅ Navigue sur la page d'accueil SeLoger
3. ✅ Effectue une recherche d'annonces
...
7. ✅ Sauvegarde les résultats dans `data/api_exploration/seloger/`
...
cat data/api_exploration/seloger/endpoints_*.json | jq '.[] | {url, status, has_json}'

# Voir les cookies importants
cat data/api_exploration/seloger/cookies_*.json | jq '.[] | select(.name | contains("session") or contains("token"))'

# Voir les tokens d'authentification
cat data/api_exploration/seloger/tokens_*.json
```
...
ls -lh data/api_exploration/seloger/
```
...
Une fois les endpoints identifiés, adaptez `seloger_api_client.py` :
...
Pour voir plus de détails pendant l'exploration, modifiez `explore_seloger_api.py` :
...
```python
# Exemple : capturer uniquement les requêtes vers api.seloger.com
if 'api.seloger.com' not in url:
...
1. **Base URL de l'API :**
   - `https://api.seloger.com`
   - `https://www.seloger.com/api`
   - `https://ws.seloger.com`

2. **Structure des endpoints :**
   - RESTful : `/api/v1/properties/{id}`
   - GraphQL : `/graphql`
   - WebSocket : `ws://ws.seloger.com/...`
...
- Respectez les Terms of Service de SeLoger
...
1. **Créer le client API** (`seloger_api_client.py`)
...
3. **Documenter l'API**
   - Créer `docs/api/seloger_api_reference.md`
...
**Besoin d'aide ?** Consultez les fichiers générés dans `data/api_exploration/seloger/` pour plus de détails.
