#!/usr/bin/env python3
"""
Test du dashboard avec session existante
"""

import asyncio
from scrape_jinka import JinkaScraper

async def test_dashboard_connected():
    """Test du dashboard avec session existante"""
    print("🏠 TEST DASHBOARD CONNECTÉ")
    print("=" * 40)
    print("Assure-toi d'être connecté à Jinka dans Chrome")
    print()
    ...
    try:
        ...
        if "sign/in" in current_url:
            print("❌ Redirection vers login - session non partagée")
            print("💡 Tu dois te connecter dans ce nouveau navigateur aussi")
            return False
        else:
...
if __name__ == "__main__":
    asyncio.run(test_dashboard_connected())
